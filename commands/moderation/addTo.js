...
module.exports.run = ( client, message, args ) => {
    ...
    if( `<@!${user.user.id}>` === args[0] ) {
        ...
        args.forEach( roleName => {
            ...
            if(role) {
                if( user.roles.cache.has( role.id ) )       return message.channel.send(`${user.toString()} a déjà le rôle ${role}.`);

                if( role.permissions.has('ADMINISTRATOR') ) return message.channel.send("Je n'ai pas le droit d'ajouter ce rôle :(");

                user.roles.add(role)
                    .then( m => message.channel.send(`${user.toString()} a maintenant le rôle ${role} !`) )
